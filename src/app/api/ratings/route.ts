...
export async function GET(request: NextRequest) {
  ...
  const data = await col
    ...
    .collection("ratings")
    .get()
    .then((col) => col.docs.map((doc) => doc.data()));
...
export async function POST(request: NextRequest) {
  ...
  const col = db
    .collection("maps")
    .doc(body.map_id.toString())
    .collection("ratings");

  if (!(await db.collection("users").doc(body.user_id.toString()).get()).exists)
    return Response.json({ status: 404, message: "User does not exist" });

  if (!(await db.collection("maps").doc(body.map_id.toString()).get()).exists)
    return Response.json({ status: 404, message: "Map does not exist" });

  col.doc(body.user_id.toString()).set(rating);

  const totalRatingNum: number = await col
    .count()
    .get()
    .then((col) => col.data().count);

  const ratingSum: number = await col
    .aggregate({ sum: AggregateField.sum("rating") })
    .get()
    .then((snp) => snp.data().sum);
